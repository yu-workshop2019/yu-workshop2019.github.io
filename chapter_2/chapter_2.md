...
`$chromium-browser &`  
または、デスクトップ左上の地球のアイコンをクリックしてもよい。
3. 学内LANの認証を通す。ブラウザのURL入力欄に`ic.cc.yamaguchi-u.ac.jp`と入力。  
認証ページに自分のユーザIDとパスワードを入力する。  
4. 何か適当なページが開けるかやってみる。  

※学内からRaspberry Piでネットワークに関連した作業（apt-get、ブラウザによるWebページ閲覧など）をするには、Raspberry Piを起動するたびに以下の手順が必要です。  
...
### 参考:chromiumの検索エンジンを変更する

1.chromiumを起動し、右上のボタン（縦に3つの丸が並んでいるアイコン）をクリックする。  
2.「設定」の項目をクリックする。  
3.下にスクロールし、「検索エンジン」の項目から、「Google」を選択する（Yahoo!が好きな人はYahoo!でも良い）。

これで検索エンジンがGoogleとなり、「山口大学工学部ワークショップ」と検索するとこのページが検索結果に表示されるようになる。
